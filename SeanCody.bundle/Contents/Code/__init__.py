...
class SeanCody(Agent.Movies):
    ...
    def search(self, results, media, lang, manual):
        ...
        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - media.title -  %s',
                 media.title)
        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - media.items[0].parts[0].file '
                 '-  %s', media.items[0].parts[0].file)
        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - media.primary_metadata.title '
                 '-  %s', media.primary_metadata.title)
        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - media.items -  %s',
                 media.items)
        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - media.filename -  %s',
                 media.filename)
        ...
        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - File Path: %s' %
                 path_and_file)
        ...
        if final_dir not in folder_list:
            self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - Skipping %s because the '
        ...
        movie_url = BASE_TOUR_MOVIE_URL % (groups['clip_number'],
                                           movie_url_name)

        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - Video URL: %s' % movie_url)
        try:
            html = HTML.ElementFromURL(movie_url, sleep=REQUEST_DELAY)
        except:
            self.Log(PLUGIN_LOG_TITLE + " - SEARCH - Title not "
                     "found: %s" % movie_url)
            return

        movie_name = html.xpath('//*[@id="player-wrapper"]/div/h1/text()')[0]
        self.Log(PLUGIN_LOG_TITLE + ' - SEARCH - title: %s' % movie_name)
        results.Append(
            MetadataSearchResult(id=movie_url, name=movie_name, score=100,
                                 lang=lang))
        return

    def fetch_summary(self, html, metadata):
        raw_about_text = html.xpath('//*[@id="description"]/p')
        self.Log(PLUGIN_LOG_TITLE + ' - UPDATE - About Text - RAW %s',
                 raw_about_text)
    ...
    def fetch_release_date(self, html, metadata):
        release_date = \
            html.xpath('//*[@id="player-wrapper"]/div/span/time/'
                       'text()')[0].strip()
        self.Log(PLUGIN_LOG_TITLE + ' - UPDATE - Release Date - '
                 'New: %s' % release_date)
    ...
    def fetch_roles(self, html, metadata):
        ...
        htmlcast = html.xpath('//*[@id="scroll"]/div[2]/ul[2]/li/a/'
                              'span/text()')
        self.Log(PLUGIN_LOG_TITLE + ' - UPDATE - cast: "%s"' %
                 htmlcast)
    ...
    def fetch_genre(self, html, metadata):
        ...
        genres = \
            html.xpath('//*[@id="scroll"]/div[2]/ul[1]/li/a/text()')
        self.Log(PLUGIN_LOG_TITLE + ' - UPDATE - video_genres: "%s"' %
                 genres)
        for genre in genres:
            genre = genre.strip()
            if (len(genre) > 0):
    ...
    def update(self, metadata, media, lang, force=False):
        ...
        self.Log(PLUGIN_LOG_TITLE + ' - UPDATE - Movie URL: %s' % metadata.id)
